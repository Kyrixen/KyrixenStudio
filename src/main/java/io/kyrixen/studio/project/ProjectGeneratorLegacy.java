...
public class ProjectGeneratorLegacy {
    ...
    private static void copyAndExtract(String dirPath) throws IOException {
        ...
        try (InputStream in = ProjectGenerator.class.getResourceAsStream("/legacy_template.zip")) {
            if (in == null) throw new IOException("legacy_template.zip not found");
            Files.copy(in, Paths.get(dirPath, "legacy_template.zip"));
        }
        
        unzip(Paths.get(dirPath + "/legacy_template.zip"), dirPath);
        Files.deleteIfExists(Paths.get(dirPath + "/legacy_template.zip"));
    ...
    private static void replacePackages(Project project) throws IOException {
        ...
        String[] modules = {"core", "lwjgl2", "android", "html"};
    ...
    private static void renameMain(Project project) throws IOException {
        ...
        Files.move(mainPath.resolve(("GdxGame.gwt.xml")), mainPath.resolve(project.getName() + ".gwt.xml"));
    ...
    private static void setupSettings(Project project) throws IOException {
        ...
        if(project.getPlatforms().contains("Desktop")) include("lwjgl2", gradleSettings);
        if(project.getPlatforms().contains("Web")) include("html", gradleSettings);
    ...
    private static void deleteUnusedModules(Project project) throws IOException {
        ...
        replaceLine(gradleProps, "GWT", project.getPlatforms().contains("Web"));
        replaceLine(gradleProps, "LWJGL2", project.getPlatforms().contains("Desktop"));

        replaceLine(buildGradle, "ANDROID", project.getPlatforms().contains("Android"));
        replaceLine(buildGradle, "GWT", project.getPlatforms().contains("Web"));
        replaceLine(buildGradle, "LWJGL2", project.getPlatforms().contains("Desktop"));
    
        deleteIfMissing(project, "Desktop", "lwjgl2");
        deleteIfMissing(project, "Web", "html");
